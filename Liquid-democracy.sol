*
 * 	Nombre: Liquid-democracy.sol
 *	Version:	v0.1
 *	Autor:	Jorge Martin
 *	Fecha:	diciembre 2017
 *	Descripcion:		Liquid-democracy.sol
 *                  Empezaremos desde cero a intentar crear un contrato tipo,
 *                  add personificacion
 *          
 *                any voter can be a potential delegate: instead of voting the candidate you want, 
 *                you just say which voter you trust to handle this decision for you. 
 *                Your voting weight is delegated to them and they can in turn delegate it to another voter they trust 
 *                and so on. The end result should be that the most voted account is one that has trust connections 
 *                to the largest amount of voters.
 *                his means that if you don't have urgency, one or two signatures might be all you need to execute 
 *                any transaction. But if a single key is compromised, other keys can delay that transaction for 
 *                months or year or even stop it from being executed.
 *
 *                 
*                 
*                 
