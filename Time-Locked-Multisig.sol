*
 * 	Nombre: Time-Locked-Multisig.sol
 *	Version:	v0.1
 *	Autor:	Jorge Martin
 *	Fecha:	diciembre 2017
 *	Descripcion:		Time-Locked-Multisig.sol
 *                  Empezaremos desde cero a intentar crear un contrato tipo TokenERC20,
 *                  add personificacion
 *          
 *                Sometimes time can also be used as a great security mechanism. 
 *                The following code is based on the congress DAO but with a different twist. 
 *                Instead of every action requiring the approval of an X number of members, 
 *                instead any transactions can be initiated by a single member, 
 *                but they all will require a minimum amount of delay before they can be executed, 
 *                which varies according to the support that transaction has. 
 *                The more approvals a proposal has, the sooner it can be executed. A member can vote against a transaction,
 *                which will mean that it will cancel one of the other approved signatures.
 *
 *                his means that if you don't have urgency, one or two signatures might be all you need to execute any transaction. But if a single key is compromised, other keys can delay that transaction for months or year or even stop it from being executed.
 *
 *                 HOW IT WORKS
*                 A transaction that has been approved by all keys can be executed after ten minutes 
*                 (this amount is configurable), and the amount of time it requires doubles every time 
*                 for every 5% of members who don't vote (and quadruples if they activelly vote against). 
*                 If it's a simple ether transaction, the transaction is executed as soon as a vote of support 
*                  puts it under the required time, but a more complex transaction will require it to be manually 
*                 executed with the correct bytecode. These are the default values, but this can be set differently 
